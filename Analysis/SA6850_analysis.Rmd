...
```{r}
# cluster on features
features$phenotype_clusters_4 = as.factor(kmeans(features[,c("growth_in_supernatant", "growth_in_TSB", "log_average_STX", "log_average_H2O2_survival","Hemolysis_score")],4)$cluster)
```

```{r}
ggplot(features,
       aes(x = PC1, y= PC2,color=phenotype_clusters_4,shape=Condition))+
  geom_point()
...
```{r}
ggplot(features, aes(x = phenotype_clusters_4, fill=Condition))+
  geom_bar()
```
get the id of wrong clustering TSB clones
```{r}
features%>%
  # filter(Condition == "TSB" & phenotype_clusters_2 == 2 & log_STX_clusters_2 == 1 & log_H2O2_clusters_2 == 1) %>%
  filter(Condition == "TSB" & phenotype_clusters_2 == 2) %>%
  select(ID)
```
