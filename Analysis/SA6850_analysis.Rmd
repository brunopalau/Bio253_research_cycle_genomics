...
supernatant_wrong_clustering <- features%>%
  filter(Condition == "supernatant" & phenotype_clusters == 2) %>%
  select(ID, Condition)
...
pdf("../Out/SA6850/heatmap_SA6850.pdf", width = 16, height = 8)
...
SA6850_hm <- relative_hm %v% aboslute_hm
...
```{r}
write.csv(features%>%select(c("ID","Strain","Condition","phenotype_clusters", "PC1", "PC2")),"../Out/SA6850/cluster_results_SA6850.csv")
```
...
write.csv(features%>%
            filter(phenotype_clusters == 1 | Condition == "supernatant")%>%
            select(c("ID","Strain","Condition","cond_clusters","PC1", "PC2")),"../Out/SA6850/condition_results_SA6850.csv")
```
